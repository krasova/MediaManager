...
public class PictureResource {
   ...
   * @return the ResponseEntity with status 201 (Created) and with body the new picture, or with
   *         status 400 (Bad Request) if the picture has already an ID
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PostMapping("/pictures")
  @Timed
  public ResponseEntity<Picture> createPicture(@Valid @RequestBody Picture picture)
      throws URISyntaxException {
    log.debug("REST request to save Picture : {}", picture);
    if (picture.getId() != null) {
      throw new BadRequestAlertException("A new picture cannot already have an ID", ENTITY_NAME,
          "idexists");
    }
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the updated picture, or with
   *         status 400 (Bad Request) if the picture is not valid, or with status 500 (Internal
   *         Server Error) if the picture couldn't be updated
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PutMapping("/pictures")
  @Timed
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the picture, or with status 404
   *         (Not Found)
   */
  ...
  /**
   * SEARCH /_search/pictures?query=:query : search for the picture corresponding to the query.
   *
  ...
  @Timed
  public ResponseEntity<List<Picture>> searchPictures(@RequestParam String query,
      Pageable pageable) {
    ...
    HttpHeaders headers =
        PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/pictures");
    return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
  }
