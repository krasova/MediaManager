package com.ostrovskiy.media.security;

import com.ostrovskiy.media.domain.PersistentToken;
import com.ostrovskiy.media.repository.PersistentTokenRepository;
import com.ostrovskiy.media.repository.UserRepository;
import com.ostrovskiy.media.service.util.RandomUtil;
 ...
 * </ul>
 * <p>
 * Please note that it allows the use of the same token for 5 seconds, and this value stored in a
 * specific cache during that period. This is to allow concurrent requests from the same user:
 * otherwise, two requests being sent at the same time could invalidate each other's token.
 * <p>
 * This is inspired by:
 * <ul>
 ...
 * <li><a href="https://github.com/blog/1661-modeling-your-app-s-user-session">GitHub's "Modeling
 * your App's User Session"</a></li>
 * </ul>
 * <p>
 * The main algorithm comes from Spring Security's PersistentTokenBasedRememberMeServices, but this
 * class couldn't be cleanly extended.
 */
@Service
public class PersistentTokenRememberMeServices extends AbstractRememberMeServices {
  ...
  private void addCookie(PersistentToken token, HttpServletRequest request,
      HttpServletResponse response) {
    setCookie(new String[] {token.getSeries(), token.getTokenValue()}, TOKEN_VALIDITY_SECONDS,
        request, response);
  }
