...
const apiUrl = SERVER_API_URL + '/api/pictures';
const apiSearchUrl = SERVER_API_URL + '/api/_search/pictures';

// Actions

export const getSearchEntities: ICrudSearchAction<IPicture> = query => ({
  type: ACTION_TYPES.SEARCH_PICTURES,
  payload: axios.get<IPicture>(`${apiSearchUrl}?query=` + query)
});
